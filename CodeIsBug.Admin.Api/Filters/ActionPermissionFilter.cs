...
namespace CodeIsBug.Admin.Api.Filters
{
    /// <summary>
    /// API授权判断
    /// </summary>
    public class ActionPermissionFilter : ActionFilterAttribute//, IAsyncActionFilter
    {
        ...
        public override Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            ...
            //    {
            //        context.Result = new Microsoft.AspNetCore.Mvc.JsonResult(new { code = ResultCode.FORBIDDEN, msg = "演示模式 , 不允许操作" });
            //    }
            //    if (!HasPermi && !Permission.Equals("common"))
            //    {
            //        logger.Info($"用户{info.UserName}没有权限访问{url}，当前权限[{Permission}]");
            //        JsonResult result = new(new Result()
            //        {
